...
var Target = function(planet,score) {
	if(score == null) score = 0;
	this.planet = planet;
	this.score = score;
};
Target.__name__ = true;
var Targets = function(origin) {
	this.origin = origin;
	this.targets = new Array();
};
Targets.__name__ = true;
Targets.prototype = {
...
VelociTeamIA.prototype = $extend(WorkerIA.prototype,{
	...
	,getPlanetsScore: function(context) {
		...
		var ennemyPlanets = com.tamina.planetwars.utils.GameUtil.getEnnemyPlanets(this.id,context);
		var allTargets = new Array();
		var _g1 = 0, _g = myPlanets.length;
		while(_g1 < _g) {
			var i = _g1++;
			var targets = new Targets(myPlanets[i]);
			var _g3 = 0, _g2 = ennemyPlanets.length;
			while(_g3 < _g2) {
				var j = _g3++;
				targets.addTarget(this.getPlanetScore(myPlanets[i],ennemyPlanets[j]));
			}
